...
static void MixColumns(uint8_t* s) {
    for (int i = 0; i < 16; i += 4) {
        ...
        uint8_t xb = xtime(b ^ c);
        uint8_t xc = xtime(c ^ d);
...
static void InvMixColumns(uint8_t* s) {
    for (int i = 0; i < 16; i += 4) {
        ...
        s[i]   = (uint8_t)(0x0e*a ^ 0x0b*b ^ 0x0d*c ^ 0x09*d);
        s[i+1] = (uint8_t)(0x09*a ^ 0x0e*b ^ 0x0b*c ^ 0x0d*d);
        s[i+2] = (uint8_t)(0x0d*a ^ 0x09*b ^ 0x0e*c ^ 0x0b*d);
        s[i+3] = (uint8_t)(0x0b*a ^ 0x0d*b ^ 0x09*c ^ 0x0e*d);
    }
}
...
void aes256_cbc_encrypt(uint8_t* data, size_t len, const uint8_t iv[16]) {
    ...
    for (size_t i = 0; i < len; i += 16) {
        ...
        aes256_encrypt_block(&data[i]);
        memcpy(buf, &data[i], 16);
    }
}
