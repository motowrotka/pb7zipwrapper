#include "wrapper.h"

#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <vector>
#include <stdint.h>

extern "C" {
#include "LzmaLib.h"
}

// ------------------------------------------------------------
//  AES‑256 CTR – minimalna, wbudowana implementacja
// ------------------------------------------------------------

static const uint8_t sbox[256] = {
  
};

static uint8_t roundKey[240];

static const uint8_t Rcon[15] = {
  0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36,0x6C,0xD8,0xAB,0x4D
};

static void KeyExpansion(const uint8_t key[32]) {
    memcpy(roundKey, key, 32);
    uint8_t temp[4];
    int bytesGenerated = 32;
    int rconIter = 1;

    while (bytesGenerated < 240) {
        memcpy(temp, &roundKey[bytesGenerated - 4], 4);

        if (bytesGenerated % 32 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
            temp[0] ^= Rcon[rconIter++];
        }

        for (int i = 0; i < 4; i++) {
            roundKey[bytesGenerated] = roundKey[bytesGenerated - 32] ^ temp[i];
            bytesGenerated++;
        }
    }
}

static void AddRoundKey(uint8_t* state, int round) {
    for (int i = 0; i < 16; i++)
        state[i] ^= roundKey[round * 16 + i];
}

static void SubBytes(uint8_t* state) {
    for (int i = 0; i < 16; i++)
        state[i] = sbox[state[i]];
}

static void ShiftRows(uint8_t* state) {
    uint8_t temp;

    temp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = temp;

    temp = state[2];
    state[2] = state[10];
    state[10] = temp;
    temp = state[6];
    state[6] = state[14];
    state[14] = temp;

    temp = state[3];
    state[3] = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = temp;
}

static void MixColumns(uint8_t* s) {
    for (int i = 0; i < 16; i += 4) {
        uint8_t a = s[i];
        uint8_t b = s[i+1];
        uint8_t c = s[i+2];
        uint8_t d = s[i+3];

        uint8_t e = a ^ b ^ c ^ d;

        uint8_t xa = (uint8_t)((a ^ b) << 1) ^ (((a ^ b) & 0x80) ? 0x1B : 0);
        uint8_t xb = (uint8_t)((b ^ c) << 1) ^ (((b ^ c) & 0x80) ? 0x1B : 0);
        uint8_t xc = (uint8_t)((c ^ d) << 1) ^ (((c ^ d) & 0x80) ? 0x1B : 0);
        uint8_t xd = (uint8_t)((d ^ a) << 1) ^ (((d ^ a) & 0x80) ? 0x1B : 0);

        s[i]   ^= e ^ xa;
        s[i+1] ^= e ^ xb;
        s[i+2] ^= e ^ xc;
        s[i+3] ^= e ^ xd;
    }
}

static void aes256_encrypt_block(uint8_t block[16]) {
    AddRoundKey(block, 0);

    for (int round = 1; round <= 13; round++) {
        SubBytes(block);
        ShiftRows(block);
        MixColumns(block);
        AddRoundKey(block, round);
    }

    SubBytes(block);
    ShiftRows(block);
    AddRoundKey(block, 14);
}

// ------------------------------------------------------------
//  AES‑CTR: encrypt = decrypt = XOR z keystreamem
// ------------------------------------------------------------

static void aes256_ctr_crypt(uint8_t* data, size_t len,
                             const uint8_t key[32],
                             uint8_t iv[16])
{
    KeyExpansion(key);

    uint8_t counter[16];
    memcpy(counter, iv, 16);

    uint8_t keystream[16];

    for (size_t i = 0; i < len; i += 16) {
        memcpy(keystream, counter, 16);
        aes256_encrypt_block(keystream);

        size_t block = (len - i >= 16) ? 16 : (len - i);

        for (size_t j = 0; j < block; j++)
            data[i + j] ^= keystream[j];

        // inkrementacja licznika (big endian)
        for (int c = 15; c >= 0; c--) {
            counter[c]++;
            if (counter[c] != 0) break;
        }
    }
}

// ------------------------------------------------------------
//  Prosty hash hasła → 32 bajty (placeholder SHA‑256)
// ------------------------------------------------------------

static void sha256_simple(const char* password, uint8_t out[32]) {
    memset(out, 0, 32);
    size_t len = strlen(password);
    for (size_t i = 0; i < len && i < 32; i++)
        out[i] = (uint8_t)password[i];
}

// ------------------------------------------------------------
//  Encrypt + Compress (LZMA + AES‑CTR)
// ------------------------------------------------------------

int __stdcall EncryptAndCompress(const char* inFile,
                                 const char* outFile,
                                 const char* password)
{
    FILE* fIn = fopen(inFile, "rb");
    if (!fIn) return 1;

    fseek(fIn, 0, SEEK_END);
    long inSizeLong = ftell(fIn);
    fseek(fIn, 0, SEEK_SET);
    if (inSizeLong < 0) {
        fclose(fIn);
        return 1;
    }

    SizeT inSize = (SizeT)inSizeLong;

    std::vector<uint8_t> inBuf(inSize);
    if (fread(inBuf.data(), 1, inSize, fIn) != inSize) {
        fclose(fIn);
        return 1;
    }
    fclose(fIn);

    // LZMA compress
    SizeT outBufSize = inSize + inSize / 3 + 128;
    std::vector<uint8_t> comp(outBufSize);
    SizeT compSize = outBufSize;

    uint8_t props[5];
    SizeT propsSize = 5;

    int res = LzmaCompress(
        comp.data(), &compSize,
        inBuf.data(), inSize,
        props, &propsSize,
        5, 0, 3, 0, 2, 32, 1
    );
    if (res != SZ_OK) return 2;

    comp.resize(compSize);

    // AES‑CTR
    uint8_t key[32];
    sha256_simple(password, key);

    uint8_t iv[16];
    for (int i = 0; i < 16; i++)
        iv[i] = (uint8_t)(rand() & 0xFF);

    aes256_ctr_crypt(comp.data(), compSize, key, iv);

    // zapis pliku
    FILE* fOut = fopen(outFile, "wb");
    if (!fOut) return 3;

    fwrite("PBCRYPT2", 1, 8, fOut);

    uint8_t ivLen = 16;
    fwrite(&ivLen, 1, 1, fOut);
    fwrite(iv, 1, 16, fOut);

    uint8_t propsLen = (uint8_t)propsSize;
    fwrite(&propsLen, 1, 1, fOut);
    fwrite(props, 1, propsSize, fOut);

    uint64_t origSize64 = (uint64_t)inSize;
    fwrite(&origSize64, 1, 8, fOut);

    uint64_t compSize64 = (uint64_t)compSize;
    fwrite(&compSize64, 1, 8, fOut);

    fwrite(comp.data(), 1, compSize, fOut);

    fclose(fOut);
    return 0;
}

// ------------------------------------------------------------
//  Decrypt + Decompress (AES‑CTR + LZMA)
// ------------------------------------------------------------

int __stdcall DecryptAndDecompress(const char* inFile,
                                   const char* outFile,
                                   const char* password)
{
    FILE* fIn = fopen(inFile, "rb");
    if (!fIn) return 1;

    char magic[8];
    if (fread(magic, 1, 8, fIn) != 8) {
        fclose(fIn);
        return 1;
    }
    if (memcmp(magic, "PBCRYPT2", 8) != 0) {
        fclose(fIn);
        return 2;
    }

    uint8_t ivLen = 0;
    fread(&ivLen, 1, 1, fIn);
    if (ivLen != 16) {
        fclose(fIn);
        return 3;
    }

    uint8_t iv[16];
    fread(iv, 1, 16, fIn);

    uint8_t propsLen = 0;
    fread(&propsLen, 1, 1, fIn);
    if (propsLen != 5) {
        fclose(fIn);
        return 4;
    }

    uint8_t props[5];
    fread(props, 1, 5, fIn);

    uint64_t origSize64 = 0;
    fread(&origSize64, 1, 8, fIn);

    uint64_t compSize64 = 0;
    fread(&compSize64, 1, 8, fIn);

    SizeT origSize = (SizeT)origSize64;
    SizeT compSize = (SizeT)compSize64;

    std::vector<uint8_t> comp(compSize);
    if (fread(comp.data(), 1, compSize, fIn) != compSize) {
        fclose(fIn);
        return 1;
    }
    fclose(fIn);

    // AES‑CTR decrypt
    uint8_t key[32];
    sha256_simple(password, key);

    aes256_ctr_crypt(comp.data(), compSize, key, iv);

    // LZMA decompress
    std::vector<uint8_t> out(origSize);
    SizeT outProcessed = origSize;
    SizeT srcLen = compSize;

    int res = LzmaUncompress(
        out.data(), &outProcessed,
        comp.data(), &srcLen,
        props, 5
    );
    if (res != SZ_OK) return 5;

    FILE* fOut = fopen(outFile, "wb");
    if (!fOut) return 6;

    fwrite(out.data(), 1, outProcessed, fOut);
    fclose(fOut);

    return 0;
}
